...
import { StartEnd } from "./TerminologyFlexboxStartend.styles";

function TerminologyFlexboxStartEnd({ startend }) {
  const tlStartEnd = gsap.timeline();
  const startendLeftRef = useRef();
  const startendRightRef = useRef();

  function animStartEnd() {
    gsap.set(startendLeftRef.current, { opacity: 0, x: -50 });
    gsap.set(startendRightRef.current, { opacity: 0, y: 50 });
    tlStartEnd.to(startendLeftRef.current, {
      duration: 0.3,
      opacity: 1,
      x: 0,
      stagger: 0.2,
    ...
    tlStartEnd.to(startendRightRef.current, {
      duration: 0.3,
      opacity: 1,
      y: 0,
      stagger: 0.2,
